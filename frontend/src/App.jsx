...
import { BarChart, Bar, XAxis, YAxis, Tooltip, CartesianGrid,PieChart,
...
function App() {
  ...
  return (
    ...
    <div className={darkMode ? "min-h-screen bg-[#020c2b] flex flex-col items-center gap-6 p-10 text-white" : "min-h-screen bg-gray-200 flex flex-col items-center gap-6 p-10 text-black"}>
      ...
      <input
        className="border p-3 rounded-lg bg-white text-black w-80"
        type="text"
  ...
  className="bg-white text-black p-1 rounded-lg w-80 mb-5 border border-gray-400"
>
...
<br></br>
      <button onClick={addCompany}
        className="bg-blue-500 hover:bg-blue-700 px-4 py-3 rounded-lg"
        >
        Add Company
      </button>
...
<button
  onClick={uploadCSV}
  className="bg-green-500 hover:bg-green-700 px-5 py-3 rounded-lg text-white mb-5"
>
  Upload CSV
</button>
...
>
  ...
  >
    ...
    {companies.map((entry, index) => (
      ...
      <Cell
        key={index}
        fill={["#06B6D4", "#6366F1", "#EC4899", "#22C55E"][index % 4]}
      />
     ...
     {companies.length === 0 && (
  <h2 className="text-white text-2xl mt-10 text-center">
    No Companies Found
  </h2>
)}
    ...
    {companies
       .filter((company) =>
        ...
        .map((company) => (
        <div key={company.id}
        className="bg-white text-black p-7 rounded-xl my-4 w-full max-w-xl shadow-lg mx-auto flex-col gap-4"
        ...
        >
         ...
         <button
          ...
          className="bg-green-500 hover:bg-green-700 px-4 py-2 rounded-lg mt-4"
          >
         {approvedCompanies.includes(company.id)
         ? "Approved"
         : "Approve"}
          </button>

          <button onClick={() => deleteCompany(company.id)}
             className="bg-red-500 hover:bg-red-700 text-white px-4 py-2 rounded-lg mt-3"
            >
            Delete
          </button>
          ...
          <input
           ...
           value={editId === company.id ? editIndustry : ""}
           onChange={(e) => {
         ...
         <input
           className="border p-2 rounded-lg bg-white text-black w-full"
           type="text"
           placeholder="New Company Name"
           value={editId === company.id ? editName : ""}
           onChange={(e) => {
          setEditId(company.id);
        ...
        <button 
        className="bg-yellow-400 hover:bg-yellow-500 text-white px-3 py-1 rounded-lg mt-3"
        onClick={() => updateCompany(company.id, company.name,company.industry)}>
         Update
        </button>
